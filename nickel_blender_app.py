...
def detect_header_and_delim(file_bytes):
    ...
    # find header within first 100 lines
    header_idx = 0
...
def optimize_single_cargo(norm_df, T, G, eps, x_min, prefer_min_lots=True):
    ...
    try:
        ...
        if eps is None:
            ...
            prob += pl.lpSum(x[lot["id"]] * lot["ni"] for lot in lots) <= (G + eps) * T
        if prefer_min_lots:
            prob.setObjective(pl.lpSum(y[lot["id"]] for lot in lots))
        else:
            dev = pl.LpVariable("dev", lowBound=0)
            avg_numer = pl.lpSum(x[lot["id"]] * lot["ni"] for lot in lots)
            prob.setObjective(dev)
            prob += avg_numer >= (G) * T - dev * T
            prob += avg_numer <= (G) * T + dev * T
        ...
        # Greedy fallback: rank by Ni closeness to G, enforce x_min, fill cargo
        remaining = T
        x_map = {}
        candidates = [lot for lot in lots if lot["avail"] >= x_min]
        # if eps is None we prefer >= G, else closest-to-target
        if eps is None:
            candidates.sort(key=lambda l: (-1 if l["ni"] >= G else 1, -l["ni"]))  # take high-Ni first
        else:
            candidates.sort(key=lambda l: abs(l["ni"] - G))
...
def balance_two_cargoes(norm_df, T, floor, x_min):
    ...
    for lot, ni, avail in high:
        rem = avail
        while rem >= x_min:
            # pick cargo needing Ni most (lowest surplus)
    ...
    for lot, ni, avail in low:
        rem = avail
        while rem >= x_min:
            # pick cargo with higher surplus and remaining > 0
            idx = 0 if cargoes[0]["surplus"] >= cargoes[1]["surplus"] else 1
    ...
    for cg in cargoes:
        ...
        if df_out.empty:
            outs.append((df_out, None, False))
            continue
        total = df_out["WMT_to_load"].sum()
        avg = (df_out["WMT_to_load"] * df_out["Ni_%"]).sum() / total
        meets = (abs(total - T) < 1e-6) and (avg >= floor - 1e-12)
...
# UI
# =============================
st.set_page_config(page_title="Nickel Blending Optimizer", layout="wide")
st.title("🛠️ Nickel Blending Optimizer")

st.sidebar.header("Inputs")
uploaded = st.sidebar.file_uploader("Upload PSI CSV/XLSX", type=["csv","xlsx"])

hint = st.sidebar.expander("Column mapping (optional)")
with hint:
...
mode = st.sidebar.selectbox("Mode", ["Single Cargo (target ± tolerance)","Single Cargo (Ni ≥ floor)","Max Cargo (Ni ≥ floor)","Balanced Two Cargoes (Ni ≥ floor)"])
target_ni = st.sidebar.number_input("Target Ni % / Floor", min_value=0.0, max_value=5.0, value=1.40, step=0.01)
tolerance = st.sidebar.number_input("Tolerance ±%", min_value=0.0, max_value=1.0, value=0.02, step=0.005, help="Used only in 'target ± tolerance' mode; set 0 for exact/strict.")
x_min = st.sidebar.number_input("Minimum per used lot (WMT)", min_value=0, max_value=5000, value=300, step=50)
prefer_min_lots = st.sidebar.checkbox("Prefer fewer lots (tie-breaker)", value=True)
...
if uploaded is not None:
    try:
        ...
        if {"LotID","WMT_available","Ni"}.issubset(norm_df.columns):
            if mode == "Single Cargo (target ± tolerance)":
                rows_df, kpis, solver = optimize_single_cargo(norm_df, cargo_wmt, target_ni, tolerance, x_min, prefer_min_lots)
                st.subheader("Blend Recommendation")
                st.dataframe(rows_df)
                st.write(kpis)
                st.caption(f"Solver: {solver}")
                st.download_button("Download CSV", rows_df.to_csv(index=False), file_name=f"blend_{cargo_wmt}wmt_target{target_ni}.csv")
                ...
                st.subheader("Blend Recommendation (Ni ≥ floor)")
                st.dataframe(rows_df)
                st.write(kpis)
                st.caption(f"Solver: {solver}")
                st.download_button("Download CSV", rows_df.to_csv(index=False), file_name=f"blend_{cargo_wmt}wmt_floor{target_ni}.csv")
            elif mode == "Max Cargo (Ni ≥ floor)":
                sel, summary = max_cargo_ge_floor(norm_df, target_ni, x_min)
                st.subheader("Max Cargo Blend (Ni ≥ floor)")
                st.dataframe(sel)
                st.write(summary)
                st.download_button("Download CSV", sel.to_csv(index=False), file_name=f"maxcargo_floor{target_ni}.csv")
            else:  # Balanced Two Cargoes
                ...
                for i, (df_out, avg, meets) in enumerate(outs, start=1):
                    ...
                    st.download_button(f"Download Cargo {i} CSV", (df_out.to_csv(index=False) if not df_out.empty else "").encode("utf-8"),
                                       file_name=f"balanced_cargo_{i}_{cargo_wmt}wmt_floor{target_ni}.csv")
        else:
